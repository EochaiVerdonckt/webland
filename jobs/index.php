...
function getJobs($ctrl)
{
    return $ctrl->selectStatement("jobs",1);
}
...
function print_job($job){
    echo '<div class="col-lg-4">';
            echo '<div class="thumbnail">
                    <img src="'.$job['foto'].'" width="50%;" alt="">
                    <div class="caption">  
                    <h1>'.$job['functie'].'</h1>
                       '.$job['info'].'
                       <p><a href="foto.php?id='.$job['id'].'" class="btn btn-dark" role="button"><i class="fa fa-camera-retro"></i></a>
                       <a href="update.php?id='.$job['id'].'" class="btn btn-dark" role="button"><i class="fa fa-pen-fancy"></i></a>
                       <a href="remove.php?id='.$job['id'].'" class="btn btn-default" role="button"><i style="color:red" class="fa fa-times"></i></a>
                      ';

            if($job['publish']==0)
            {
                echo '<a href="hideOrPublish.php?id='.$job['id'].'" class="btn btn-light" role="button"><i class="fa fa-eye"></i></a>';
            }
            else{
                echo '<a href="hideOrPublish.php?id='.$job['id'].'" class="btn btn-light" role="button"><i class="fa fa-eye-slash"></i></a>';
            }
...
function showOptions($ctrl,$title){
    echo "<div class='row'>";
        ...
        <div class="row">';
            ...
            if($ctrl->count('jobs')==0)
            {
                echo '<h1>Wij hebben momenteel geen vacatures.</h1>';
            }
            else{
                $jobs=getJobs($ctrl);
            }
            if($ctrl->count('jobs')==1)
            {
               print_job($jobs);
            }
            if($ctrl->count('jobs')>1)
