...
// Check connection
				...
				$sql = "INSERT INTO `bill_balance`( `klantid`, `hasFree`) VALUES (".$_POST['id'].",1)";
...
// Check connection
				...
				$sql = "INSERT INTO `facto_balance`( `naam`, `bedrag`, `factuur`) VALUES ('".$_POST['inkomen']."','".$_POST['bedrag']."',".$last.")";
				if ($conn->query($sql) === TRUE) {
					$_SESSION['input']= "New record created successfully";
					$last2 = $conn->insert_id;
				...
				$this->Cell(45,6,"Factuurnummer: ".$last,1,0,'C');
			}
			else
			{
				echo '
...
<form method="post">';
				echo '<div style="margin: 12px;"><label>Omschrijving</label></div>';
				echo '<div style="margin: 12px;">';
				echo '<input type="text" name="inkomen">';
				echo "</div>";
				echo '<div style="margin: 12px;"><label>Bedrag</label></div>';
				echo '<div style="margin: 12px;">';
				echo '<input type="number" name="bedrag" step="any">';
				echo "</div>";
...
$pdf->Ln(20);
$pdf->Cell(90,6,"".$_POST["inkomen"],1,0,'L');
$pdf->Cell(90,6,$_POST["bedrag"]." EUR",1,0,'R');
