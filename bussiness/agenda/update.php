...
if($_POST)
{
    ...
    $sql="UPDATE `agenda_item` SET `start`='".$_POST['start']."',`end`='".$_POST['end']."',`titel`='".$_POST['titel']."',`omschrijving`='".$_POST['info']."' WHERE id=".$_POST['id'];
    ...
    if($_POST['client'])
    {
        ...
        $client=$client['id'];
        if($client && $client>0){
            $ctrl->updateStatement('agenda_item','klant',$client,$_POST['id']);
        }
    ...
    if($_POST['member2'])
    {
        $client=getMember($ctrl,$_POST['member2']);
        $client=$client['id'];
        $ctrl->updateStatement('agenda_item','member',$client,$_POST['id']);
        
    }
    
    if($_POST['depart2'])
    {
        $client=getDepart($ctrl,$_POST['depart2']);
        $client=$client['id'];
        $ctrl->updateStatement('agenda_item','departement',$client,$_POST['id']);
        
    }
    
    if($_POST['functie2'])
    {
        $client=getFunctie($ctrl,$_POST['functie2']);
        $client=$client['id'];
        $ctrl->updateStatement('agenda_item','functie',$client,$_POST['id']);
...
function getFunctie($ctrl,$email)
{
    $client=$ctrl->selectStatement("functies","naam='".$email."'");
    return $client;
}
...
function showOptions($ctrl,$title){
    echo "<div class='row'>";
         $ctrl->side_nav();   
         $item=$ctrl->selectStatement('agenda_item',"id=".$_GET['id']);
      ...
      echo '<input id="start" type="text" class="form-control" name="start" value="'.$item['start'].'"/>';
      echo '<div style="text-align:left;"><label>Einde</label></div>';
      echo '<input id="end" type="text" class="form-control" name="end" value="'.$item['end'].'"/>';
      echo '<div style="text-align:left;"><label>Titel</label></div>';
      echo '<input type="text" class="form-control" name="titel" value="'.$item['titel'].'"/>';
      echo '<div style="text-align:left;"><label>Klant (mag leeg zijn)</label></div>';
      
      if($item['klant']){
           echo '<input  type="hidden" class="form-control" name="secret" id="secret" value="'.$item['klant'].'"/>';
      }
      
      if($item['member']){
           echo '<input  type="hidden" class="form-control" name="secret2" id="secret2" value="'.$item['member'].'"/>';
      }

      if($item['departement']){
           echo '<input  type="hidden" class="form-control" name="secret3" id="secret3" value="'.$item['departement'].'"/>';
      }
      if($item['functie']){
           echo '<input  type="hidden" class="form-control" name="secret4" id="secret4" value="'.$item['functie'].'"/>';
      }
      
      echo '<input type="hidden" class="form-control" name="client" id="client-hidden"/>
      ...
      echo '<input type="hidden" class="form-control" name="depart" id="member-hidden"/>
        <div class="autocomplete" >
      ...
      for (i = 0; i < arr.length; i++) {
        ...
        if (arr[i]['title'].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
          ...
          b.addEventListener("click", function(e) {
              /*insert the value for the autocomplete text field:*/
              inp.value = this.getElementsByTagName("input")[0].value;
               document.getElementById("member-hidden").value = this.getElementsByTagName("input")[0].value;
              /*close the list of autocompleted values,
              (or any other open lists of autocompleted values:*/
              closeAllLists();
          });
...
$.get( "/members/api.php", function( data ) {
  autocomplete2(document.getElementById("membersInput"), data);
});
...
if(document.getElementById("secret")){
    var klant=document.getElementById("secret").value;
}
if(document.getElementById("secret2")){
   var member=document.getElementById("secret2").value;
}
if(document.getElementById("secret3")){
   var depart=document.getElementById("secret3").value;
}

if(document.getElementById("secret4")){
   var functie=document.getElementById("secret4").value;
}


if(klant){
    $.get( "/klanten/apiDetail.php?id="+klant, function( data ) {
       document.getElementById("clientsInput").value= data.title
    });
    
}

if(member){
    $.get( "/members/apiDetail.php?id="+member, function( data ) {
       document.getElementById("membersInput").value= data.info
    });
    
}

if(depart){
    $.get( "/departments/apiDetail.php?id="+depart, function( data ) {
       document.getElementById("departInput").value= data.title
    });
    
}

if(functie){
    $.get( "/functies/apiDetail.php?id="+functie, function( data ) {
       document.getElementById("functieInput").value= data.title
    });
