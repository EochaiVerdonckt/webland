...
if($_POST)
{
    ...
    if($_POST['depart2'])
    {
        ...
        $client=$client['id'];
        $ctrl->updateStatement($table,'departement',$client,$_POST['id']);
    ...
    if($_POST['functie2'])
    {
        ...
        $client=$client['id'];
        $ctrl->updateStatement($table,'functie',$client,$_POST['id']);
...
function showOptions($ctrl,$title){
    $item=$ctrl->selectStatement('members',"id=".$_GET['id']);
    echo "<div class='row'>";
         $ctrl->side_nav();   
        echo "<div class='col-md-9' style=''>";
                ...
                echo '<div style="text-align:left;"><label>Pas hier uw naam aan</label></div>';
                echo  '<input type="text" name="info" value="'.$item['info'].'" class="form-control"/>';
                 if($item['departement']){
