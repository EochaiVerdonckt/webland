...
if($_POST['comment'])
{
    ...
    $sql='UPDATE `cat_balance` SET `naam` = "'.$_POST['naam'].'",`comment` = "'.$_POST['comment'].'" WHERE id='.$_GET['id'];
...
function toonEdit()
{
    ...
    echo '
            <form method="post">
                <label>Pas je naam aan.</label>
                <input type="text" class="form-control" name="naam" value="'.$rij['naam'].'"/>
                <label>Pas je comment aan.</label>
                <textarea name="comment" class="form-control">'.$rij['comment'].'</textarea>';
    echo $extra;
