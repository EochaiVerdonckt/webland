...
if($_POST){
    ...
    $sql = "INSERT INTO `price_balance`(`bedrag`, `cat`, `naam`, `comment` ) VALUES (".$_POST['price'].",'".$_POST['categorie']."','".$_POST['naam']."','".$_POST['comment']."')";
...
function toonNew()
{
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $extra = "<select name='categorie'  class='form-control' >";

        // output data of each row
        while($row = mysqli_fetch_assoc($result)) {
            
            if(!empty($_SESSION['cat']) && $_SESSION['cat']==$row['id']){
    ...
    if(!empty($_POST['id'])){
        echo '<input type="hidden" name="item" value="'.$_POST['id'].'">';
    }
    ...
    echo '
                ...
                <input type="hidden" step="any" name="categorie" class="form-control" value="'.$_GET['id'].'">              
                <label>Comment</label>
