...
function showOptions($ctrl,$title){
    ...
    echo "<div class='row'>";
        ...
        echo '<div class="well text-center" style="width: 100%;margin-top:50px; padding:20px;">
                ...
                <hr /> 
                <form method="post">';
    echo '<div style="text-align:left;"><label>Kies je album</label></div>';
    echo '<select name="album" class="form-control">' ;
        foreach ($albums as &$value) {
           echo ' <option value="'.$value['id'].'">'.$value['naam'].'</option>' ;
        }
    echo '</select>';
    echo '<input type="submit" class="btn btn-dark" style="margin-top:8px;" value="next"/>';
    echo '</form>';
    echo '</div>';
    echo '</div>';
    echo "</div>";
