...
if($_POST)
{
    ...
    $field="inleiding";
    $value=$_POST['inleiding'];
    $value=str_replace("'","	&#39;",$value);
    $ctrl->updateStatement($table,$field,$value,$key,1);
}

function showOptions($ctrl,$title){
    $item=$ctrl->selectStatement('news',"id=".$_GET['id']);
    echo "<div class='row'>";
         $ctrl->side_nav();   
        echo "<div class='col-md-9' style=''>";
                ...
                echo  '<input type="text" name="titel" value="'.$item['titel'].'" class="form-control"/>';
                 echo '<div style="text-align:left;"><label>Pas hier de inleiding aan</label></div>';
                 echo ' <textarea name="inleiding" id="inleiding" class="form-control">'.$item['inleiding'].'</textarea>';
                echo '<div style="text-align:left;"><label>Pas hier uw artikel aan</label></div>';
                echo '
