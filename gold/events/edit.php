...
if($_POST){
     ...
     $sql = "UPDATE `events` SET `titel`='".$_POST['titel']."',`info`='".$_POST['omschrijving']."',`datum`='".$_POST['date']."' WHERE id=".$_GET['product'];
...
function getCompanyData($ctrl){
    ...
    // Create connection
    $conn = $ctrl->getConnection();
    $sql = "SELECT * FROM `events`  where id=".$_GET['product'];
...
function showOptions($ctrl,$title){
    ...
    echo '<div><input type="text" name="titel" class="form-control" value="'.$data['titel'].'"/></div>';
    echo '<div style="text-align:left;"><label>Datum</label></div>';
    echo '<div><input type="date" name="date" class="form-control" value="'.$data['datum'].'"/></div>';
