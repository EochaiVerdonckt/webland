...
// Check connection
    ...
    $sql = "UPDATE `promo_balance` SET `promo`='".$_POST['promo']."' WHERE id=4;";
...
// Check connection
    ...
    $sql = "UPDATE `promo_balance` SET `promo`='".$_POST['bg']."'  WHERE id=5;";
    if ($conn->query($sql) === TRUE) {
...
function toonForm(){
    ...
    echo '<label>Pas de kleur aan:</label>';
    echo '<input type="color" name="promo" class="form-control" value="'.getColor().'" />';
    
    echo '<label>Pas achtergrond aan:</label>';
    echo '<input type="color" name="bg" class="form-control" value="'.getBg().'" />';
