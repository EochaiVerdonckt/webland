        ...
        if($_POST['client2']){
            $_POST['client'] = $_POST['client2'];
                ...
                if($_POST['client'])
                {
                    ...
                    $_POST['client']=getClient($ctrl,$_POST['client']);
                    $_POST['client']=$_POST['client']['id'];
                } 
        }
     

        
        $_SESSION['bill']['klant']=$_POST['client'];
        $_SESSION['bill']['btw']=$_POST['btw'];
        $_SESSION['bill']['posten']=$_POST['posten'];
        $_SESSION['bill']['pay']=$_POST['pay'];
        $_SESSION['bill']['soort']=$_POST['soort'];
        if($_SESSION['prod']){
            header('Location: aantal.php');
               
        }
        else{
            header('Location: posten.php');
        }
        exit();
    }
...
function showOptions($ctrl,$title){
    ...
    echo "<div class='row'>";
        ...
        echo '
                      ...
                      <label>Soort</label></div>
                        <div style="text-align: left;"><input type="radio" name="soort" value="factuur" checked> factuur
                        <input type="radio" name="soort" value="kas"> Offerte
                        <input type="radio" name="soort" value="bon"> bestelbon
                 </div>
        ';
        
        if($_GET['client']){
            if(is_numeric($_GET['client'])){
               echo '<input type="hidden" class="form-control" name="client2" id="client-hidden" value="'.$_GET['client'].'"/>'; 
            }
        }
        else{
             echo '<div style="text-align:left;"><label>Klant</label></div>';
        ...
        echo '<div style="text-align: left;">
                      <label>BTW tarief</label></div>
                        <div style="text-align: left;">
                            <input type="radio" name="btw" value="21" checked> 21%
                            <input type="radio" name="btw" value="6"> 6%
                            <input type="radio" name="btw" value="vrij"> vrijstelling
                             <input type="radio" name="btw" value="marge"> BTW margeregeling
                          
                        </div>

                <div style="text-align: left;">
                      <label>Betaling</label></div>
                        <div style="text-align: left;"><input type="radio" name="pay" value="cash" checked> Betaling in cash <br>
                        <input type="radio" name="pay" value="overschrijving"> Overschrijving
                 </div>';
                 if( !is_numeric($_SESSION['prod'])){
                      echo '<div style="text-align: left;">
                      <label>Aantal posten</label>
                <input type="number" step="1" class="form-control" name="posten" required="">
                 </div>';
                 }
                 else{
                      echo '<div style="text-align: left;display:none;">
                      <label>Aantal posten</label>
                <input type="number" step="1" class="form-control" name="posten" required="" value="1">
                 </div>';
                 }
